...
I will insert a `GPX` formatted file of MBTA stations from  http://erikdemaine.org/maps/mbta/ which incluedes all the waypoints of the MBTA stations as well as routes of rapid transit lines.

I will use `read_GPX()` function from `tmaptools` package which reas `GPX` files into `sf` objects in R.
...
p_h <- p %>% 
  add_trace(
    ...
    text = ~paste(Name,
                  "\nAddress: ", Address),
    hoverinfo = "text",
    marker = list(color = 'magenta', size = 7)
  ) %>% 
  add_trace(
    data = bikes,
    x = ~Longitude,
